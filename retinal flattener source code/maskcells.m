...
imst3(:,:,i)=bwareaopen(imst2(:,:,i)>200,10*5);
imst4(:,:,i)=bwareaopen(imst3(:,:,i),800*5);
imst5(:,:,i)=imst3(:,:,i).*imcomplement(imst4(:,:,i));
imst6(:,:,i)=bwdist(imcomplement(imst5(:,:,i)));
end
imst7=(single(smooth3(imst6>4,'gaussian',9)>0).*imst6);
...
imst10=permute(uint8(imcomplement(smooth3(imst9>0.1,'gaussian',5)>0)),[3 2 1]).*imst2;